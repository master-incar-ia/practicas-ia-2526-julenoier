...
from .dataset import CIFAR10Dataset # importar el dataset para esta practica
...
def train_model(output_folder: Path, device: torch.device):
    ...
    #Las que usamos en esta practica son estas
    ...
    #Estas no usamos en esta practica
    ...
    train_loader = DataLoader(dataset_train, batch_size=64, shuffle=True, pin_memory=pin_memory)
    val_loader = DataLoader(dataset_val, batch_size=64, shuffle=False, pin_memory=pin_memory)

    # Define the model, loss function, and optimizer
    input_dim = (3,32,32) # el tamaño de la imagen, 3 canales por 32*32 que son el ancho y el alto
    output_dim = 10 # la cantidad de clases de imágenes que hay
    model = ConvolutionalNet(input_dim, output_dim).to(device)
